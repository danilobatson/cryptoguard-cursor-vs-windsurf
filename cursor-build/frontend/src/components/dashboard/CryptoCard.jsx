...
const CryptoCard = ({ 
  ...
  const handleForceRefresh = () => {
    if (isRealTime && connectionStatus === 'connected') {
  ...
  const getDataQualityWarning = () => {
    if (source === 'websocket' && connectionStatus === 'connected') {
      return null // Real-time data, no warning needed
    }
    
    if (ageMinutes > 5) {
      return {
        color: 'red',
        message: 'Data may be outdated - click GO LIVE for real-time updates'
      }
    }
    ...
    if (ageMinutes > 2) {
      return {
        color: 'yellow', 
        message: 'Using recent API data - click GO LIVE for real-time updates'
      }
    }
    ...
    >
      ...
      <Group justify="space-between" align="center" mb="md">
        <Group gap="md">
          <Box>
            ...
            <Group gap="xs" mt={2}>
              <DataFreshnessIndicator symbol={symbol} data={data} />
            </Group>
          </Box>
        </Group>
      ...
      <Stack gap="xs">
        ...
        <Group justify="space-between">
          ...
          <Tooltip label={`Source: ${source} | ${ageMinutes.toFixed(1)} minutes ago`}>
