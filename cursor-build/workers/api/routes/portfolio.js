...
async function addHolding(holdingData, env) {
  ...
  await env.CRYPTO_CACHE.put('user_portfolio', JSON.stringify(portfolio), { expirationTtl: 86400 * 365 });
...
async function calculatePortfolioSummary(portfolio, env) {
  ...
  for (const holding of portfolio) {
    try {
      ...
      const priceResponse = await fetch(`https://cryptoguard-api.cryptoguard-api.workers.dev/crypto/${holding.symbol.toLowerCase()}`);
  ...
  return {
    ...
    profit_loss_percentage: totalCost > 0 ? ((totalValue - totalCost) / totalCost) * 100 : 0
  };
}
