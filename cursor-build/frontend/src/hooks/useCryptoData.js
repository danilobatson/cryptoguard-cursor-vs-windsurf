...
const getBackendUrl = () => {
  return 'https://cryptoguard-api.cryptoguard-api.workers.dev';
}

// Real crypto data fetcher using our authenticated backend
const fetchRealCryptoData = async (symbol) => {
  const backendUrl = getBackendUrl()
  console.log(`🌙 Fetching REAL ${symbol} from backend: ${backendUrl}/crypto/${symbol}`)
  ...
  try {
    ...
    // 🔥 CRITICAL FIX: Update store immediately here instead of relying on onSuccess
...
export const useMultipleCrypto = (symbols = ['bitcoin', 'ethereum']) => {
  ...
  const queries = symbols.map(symbol =>
    useQuery({
      ...
      retry: 3,
      retryDelay: 1000,
      // Remove onSuccess - we handle store update in fetchRealCryptoData
      onError: (error) => {
  ...
  const refreshAll = async () => {
    console.log('🔄 Refreshing all crypto data from backend...')
